...
# Tourist类对数据库的表进行映射,映射数据库结构，带数据库链接信息
class Tourist(Base):
...
class Window(QWidget, Ui_Form):
    ...
    def on_pushButtonQuery_clicked(self):
        ...
        for row, tourist in enumerate(rets):
            ...
            self.tableWidget.setItem(
                row, 2, QTableWidgetItem(str(tourist.生产编号)))
            # self.tableWidget.setItem(
    ...
    def applyName(self):
        ...
        # 更新或添加到字典里
            ...
            self.sql[Tourist.name] = self.lineEditName.text().strip()  # strip()移除字符串头尾指定的字符序列
