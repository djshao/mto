...
class Machine(QWidget, Ui_wgtInput):
    ...
    def __init__(self, parent=None):
        ...
        self.tblwgtInput.itemClicked.connect(self.selectDpmt)  # 明细区点击事件,加入cmb控件
    ...
    def cmbNO_activated(self):
        ...
        res = mymdb.fetchall(
            table='order_list',
            field='序号,名称,制造标准,规格型号,材质,数量,已机加数,机加部门',
            where='生产编号='+"'"+m_no+"'"+" and 生产状态 in ('部分锻造','锻造完成','部分机加')")
        # print(res[0])
        if res[0] == ():
            return
        ...
        row = len([tup[0] for tup in res[0]])  # 获得data的行数
        col = len(res[0][0])
        ...
        col_lst = [
            '序号', '名称', '制造标准', '规格型号', '材质', '订单数量', '已机加数', '机加部门', '机加数量', '机加日期', '生产状态'
        ]
    ...
    def cmbDpmt_currentIndexChanged(self):
        ...
        if m_dpmt == '金工车间':
            res = mymdb.fetchall(
                field='distinct 生产编号',
                table='order_list',
                where="机加部门 in ('Null','金工车间')")
        # 还不完善条件 and 生产状态 in ('部分机加','锻造完成','部分锻造')")
        else:
            res = mymdb.fetchall(
                field='distinct 生产编号',
                table='order_list',
                where="生产状态 in ('锻造完成','部分锻造')"
            )
            # print(res)
        if res is None:
            return
    ...
    def on_pbtnSave_clicked(self):
        ...
        preSql = "insert into 机加交货 ("+filed+")"
        subSql = "values(%s, %s, %s, %s, %s)"
        sql = preSql+subSql  # 前后相加成一个完整的sql
        sql_1 = "INSERT INTO order_list (生产编号,序号,机加部门,已机加数,生产状态) VALUES(%s, %s, %s, %s, %s)  \
                ON DUPLICATE KEY UPDATE 机加部门=VALUES(机加部门),已机加数=VALUES(已机加数),生产状态=VALUES(生产状态)"
        # print(sql)
        param = []  # 建传入值的列表
        param_1 = []
...
class Forge(QWidget, Ui_wgtInput):
    ...
    def cmbDpmt_currentIndexChanged(self):
        ...
        m_dpmt = self.cmbDpmt.currentText()
        res = mymdb.fetchall(
            field='distinct 生产编号', table='锻造', where="锻造状态!='全部完成' and 锻造部门="+"'"+m_dpmt+"'")
        if res is None:
            return
        no_lst = [tup[0] for tup in res[0]]
        self.cmbNO.addItems(no_lst)
    ...
    def on_pbtnSave_clicked(self):
        ...
        rowcount = mymdb.insert_many(sql, param)
        # 更新-->锻造状态
        self.mymdb.updateAll(sql_1, param_1)
...
class Plan(QWidget, Ui_wgt_plan):
    ...
    def openPlanList(self, param):
        ...
        # self.mymdb = myMdb()
        gsmc = param[0][2]
        khdm = mymdb.fetchone(field='客户代码', table='客户资料表', where="公司名称 like '%"+gsmc+"%'")
        self.lineEdit_1.setText(khdm[0])
        ...
        # 导入技术要求
        m_jsyq = mymdb.fetchone(field='技术要求', table='orders', where='生产编号='+param[0][0])
    ...
    def on_pbtnSave_clicked(self):
        ...
        for row in range(rows):
            if self.tblwgt.item(row, 0) is None or self.tblwgt.item(row, 0).text() == "":
                ...
                # 更新订单明细-->生产状态=锻造排产
                mymdb.update(table='order_list', 生产状态="'锻造排产'",
                            where="生产编号="+"'"+m_scbh+"'"+" and 序号="+"'"+m_xh+"'")
                value_list = []
